...
const FormField: React.FC<FormFieldProps> = ({
  ...
  <View style={styles.fieldContainer}>
    ...
    <View style={[styles.inputContainer, error && styles.inputError, !editable && { backgroundColor: '#F6F7FB' }]}>
      <TextInput
        style={[styles.textInput, !editable && { color: '#605f5fff' }]}
        value={value}
...
const EditProfile = () => {
  ...
  const { updateProfile, loading: isUpdating } = useUser();
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      const payload: UpdateUserPayload = { name: fullName.trim() };
      if (base64Image) {
      ...
      // The backend controllers (e.g., user.controllers.ts) return JSON with an 'error' key.
      // We check for 'error' first, then 'message' (common in some middleware/libraries), then the generic error message.
      const errorMessage = 
  ...
  if (!state.user) {
    return (
      <SafeAreaView style={[styles.container, { backgroundColor: '#0085FF' }]} edges={['top','left','right']}>
          ...
          <View style={styles.profileImageContainer}>
            <View style={styles.profileImage}>
              <Image // This is the Image component from expo-image
                source={selectedImage ? { uri: selectedImage } : require("../../../assets/images/user profile.png")}
